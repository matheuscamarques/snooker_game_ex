// ============================================================================
// FILE: /assets/js/inputHandler.js
// DESCRIÇÃO: Classe para gerir todos os inputs do utilizador.
// ============================================================================
...
class InputHandler {
    ...
    handleMouseDown(e) { 
        ...
        if (e.button === 0) { // Botão esquerdo para mirar
            // CORREÇÃO: Verifica se o jogador pode atirar
            if (this.hook.el.dataset.canShoot === 'false') return;
            const worldPos = this.hook.cameraModule.screenToWorld(mousePos, this.hook.camera, this.hook.canvas); 
            this.hook.cueModule.startAiming(this.hook, worldPos); 
        } else if (e.button === 2) { // Botão direito para pan
            this.hook.camera.isPanning = true; 
            this.hook.camera.lastMouse = mousePos; 
            this.hook.canvas.style.cursor = 'grabbing'; 
    ...
    handleMouseMove(e) { 
        const mousePos = this.getMousePos(e); 
        if (this.hook.camera.isPanning) { 
            const { camera } = this.hook; 
            const cosR = Math.cos(camera.rotation), sinR = Math.sin(camera.rotation); 
            const dx = (mousePos.x - camera.lastMouse.x) / camera.zoom; 
            const dy = (mousePos.y - camera.lastMouse.y) / camera.zoom; 
            camera.pan.x -= dx * cosR + dy * sinR; 
            camera.pan.y -= dy * cosR - dx * sinR; 
            camera.lastMouse = mousePos; 
        } else if (this.hook.cueState.status === 'aiming') { 
            const worldPos = this.hook.cameraModule.screenToWorld(mousePos, this.hook.camera, this.hook.canvas); 
            this.hook.cueModule.updateAim(this.hook.cueState, worldPos); 
        } 
    }

    handleMouseUp(e) { 
        if (e.button === 2) { 
            this.hook.camera.isPanning = false; 
            this.hook.canvas.style.cursor = 'grab'; 
        } 
        if (this.hook.cueState.status === 'aiming' && e.button === 0) { 
            this.hook.cueModule.applyStrike(this.hook); 
        } 
        this.removeGlobalListeners(); 
    }
    ...
    handleWheel(e) { 
        ...
        this.hook.cameraModule.zoom(this.hook.camera, zoomFactor, this.getMousePos(e), this.hook.canvas); 
    }
    ...
    handleTouchStart(e) { 
        ...
        if (e.touches.length === 1) { 
            // CORREÇÃO: Verifica se o jogador pode atirar
            if (this.hook.el.dataset.canShoot === 'false') return;
            const worldPos = this.hook.cameraModule.screenToWorld(this.getMousePos(e.touches[0]), this.hook.camera, this.hook.canvas); 
            this.hook.cueModule.startAiming(this.hook, worldPos); 
        } else if (e.touches.length === 2) { 
            this.hook.cueState.status = 'inactive'; 
            this.hook.camera.isPanning = true; 
            this.hook.camera.lastTouchDistance = this.getTouchDistance(e.touches); 
            this.hook.camera.lastTouchMidpoint = this.getTouchMidpoint(e.touches); 
            this.addGlobalListeners(); 
        } 
    }
    ...
    handleTouchMove(e) { 
        e.preventDefault(); 
        const { camera, cueState } = this.hook; 
        if (cueState.status === 'aiming' && e.touches.length === 1) { 
            const worldPos = this.hook.cameraModule.screenToWorld(this.getMousePos(e.touches[0]), camera, this.hook.canvas); 
            this.hook.cueModule.updateAim(cueState, worldPos); 
        } else if (camera.isPanning && e.touches.length === 2) { 
            const newMidpoint = this.getTouchMidpoint(e.touches); 
            const newDistance = this.getTouchDistance(e.touches); 
            if (camera.lastTouchDistance > 0) { 
                const zoomFactor = newDistance / camera.lastTouchDistance; 
                this.hook.cameraModule.zoom(camera, zoomFactor, newMidpoint, this.hook.canvas); 
            } 
            const cosR = Math.cos(camera.rotation), sinR = Math.sin(camera.rotation); 
            const dx = (newMidpoint.x - camera.lastTouchMidpoint.x) / camera.zoom; 
            const dy = (newMidpoint.y - camera.lastTouchMidpoint.y) / camera.zoom; 
            camera.pan.x -= dx * cosR + dy * sinR; 
            camera.pan.y -= dy * cosR - dx * sinR; 
            camera.lastTouchDistance = newDistance; 
            camera.lastTouchMidpoint = newMidpoint; 
        } 
    }

    handleTouchEnd(e) { 
        if (e.touches.length < 2) this.hook.camera.isPanning = false; 
        if (e.touches.length < 1 && this.hook.cueState.status === 'aiming') { 
            this.hook.cueModule.applyStrike(this.hook); 
        } 
        this.removeGlobalListeners(); 
    }
    ...
    handleKeyDown(e) { 
        ...
        if (keyMap[e.code]) { 
            e.preventDefault(); 
            this.hook.camera.keyboardPanState[keyMap[e.code]] = true; 
        } 
    }
    ...
    handleKeyUp(e) { 
        ...
        if (keyMap[e.code]) { 
            e.preventDefault(); 
            this.hook.camera.keyboardPanState[keyMap[e.code]] = false; 
        } 
    }
    ...
    setupDPadListeners() { 
        ...
        for (const [id, direction] of Object.entries(dPadMap)) { 
            const button = this.hook.el.querySelector(`#${id}`); 
            if(button) { 
                const setPanState = (state) => { this.hook.camera.panState[direction] = state; }; 
